...
fichierIn = open("/storage1/gault/Parsivel/"+str(year)+('%02d' % month)+('%02d' % day)+".txt","r", encoding="utf8", errors='ignore') 
...
# fstart=datetime.utcnow().replace(second=0, microsecond=0)
# fend=(start-timedelta(minutes=10)).replace(second=0, microsecond=0)
start = date_time_obj
...
cbar = fig.colorbar(pc, orientation="vertical", fraction=0.046, pad=0.04).set_label('number of particules', fontsize=20)
...
ax.set_xlim(0,10)
ax.set_ylim(0,10)
#plt.plot(dfit,vrain(dfit),'g',label=u"Rain")
#plt.plot(dfit,vlumpg(dfit),'r',label=u"Snow Pellets")

plt.plot(dfit,rain(dfit),'C0',label=u"Rain", color='green')
plt.plot(dfit,SP(dfit),'C1',label=u"Snow Pellets",color='black')
#plt.plot(dfit,DR(dfit),'C2',label=u"Densely rimed aggregates")
plt.plot(dfit,RA(dfit),'C3',label=u"Dry snow",color='blue')
#plt.plot(dfit,DS(dfit),'C4',label=u"Dry snow")
#plt.plot(dfit,DE(dfit),'C5',label=u"Dendrites")
...
plt.ylabel('Fallspeed (m/s)', fontsize=20)
plt.legend(loc= 'upper left', fontsize=20)
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)

fig1 = plt.gcf()
fig1.suptitle('Last 10 minutes Gault parsivel observation: ' + datetime.now().replace(second=0,microsecond=0).isoformat(' '), size=20)
fig1.subplots_adjust(top=0.90)   
fig1.set_size_inches(15.5, 8.5)         
fig1.savefig(('/storage1/developer/Parsivel_scripts/parsivel.png'), dpi=300, bbox_inches='tight')
