...
#end=(datetime.now()+timedelta(days=1)).replace(hour=5, minute=0, second=0, microsecond=0)

# test on unique file : we want the whole period define in dataframe index
start = appended_data.index[0]
end = appended_data.index[-1]
...
resultD = pd.DataFrame([flattened_list3, flattened_list2, flattened_list1,Col4,Col5]).replace(np.nan,0).T
resultD.to_csv('/storage1/developer/Parsivel_scripts/Timeserie_Diametre.csv')
...
flattened_list2 = [y for x in Col2 for y in x]
flattened_list3 = [y for x in Col3 for y in x]
resultV = pd.DataFrame([flattened_list3, flattened_list2, flattened_list1,Col4,Col5]).replace(np.nan,0).T
resultV.to_csv('/storage1/developer/Parsivel_scripts/Timeserie_Vitesse.csv')
